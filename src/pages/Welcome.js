...
//   useEffect(() => {
//   setLoading(true); // Reset loading if code changes
//   fetch(`http://localhost:5000/api/person/${code}`)
//     .then((res) => {
//       if (!res.ok) {
...
//     .catch((err) => {
//       console.error(err);
//       setPerson(null); // This ensures your 404 UI shows up
//       setLoading(false);
//     });
// }, [code]);
...
">
  ...
  animate-fade-in-up text-xl sm:text-xl md:text-xl lg:text-2xl text-yellow-400 mb-8 lg:mb-12 md:mb-12 leading-tight font-kh">
        ...
        {/* <h2 className="sm:drop-shadow-[0_2px_3px_rgba(0,0,0,0.8)]
  sm:[-webkit-text-stroke:.1px_#ffffff] animate-fade-in-up text-xl sm:text-xl md:text-xl lg:text-2xl text-lime-100 mb-4 lg:mb-2 md:mb-2 leading-tight font-kh">
          <Typewriter text={person.name} speed={90} delay={800} />
        </h2>
<h2 className="sm:drop-shadow-[0_2px_3px_rgba(0,0,0,0.8)]
  sm:[-webkit-text-stroke:.1px_#ffffff] animate-fade-in-up text-xl sm:text-xl md:text-xl lg:text-2xl text-lime-100 mb-8 lg:mb-8 md:mb-8 leading-tight font-kh">
